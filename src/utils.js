...
const messageListener = (conn, doc, message) => {
  try {
    ...
    if (!message || message.length === 0) {
      console.warn("Received empty or null message");
      return;
    }

    console.log("Processing message of length:", message.length);
    ...
    console.error("Message length:", message?.length || "undefined");
    console.error("Message type:", typeof message);
    console.error(
      "First few bytes:",
      message
        ? Array.from(message.slice(0, Math.min(10, message.length)))
        : "N/A"
    );
    // @ts-ignore
    doc.emit("error", [err]);
...
export const setupWSConnection = (
  ...
  // listen and reply to events with enhanced validation
  conn.on("message", (message) => {
    try {
      ...
      // Additional validation - check for minimum message size
      if (uint8Array.length < 1) {
        console.warn("Message too short:", uint8Array.length, "bytes");
        return;
      }

      messageListener(conn, doc, uint8Array);
    } catch (err) {
      console.error("Error in message handler setup:", err);
